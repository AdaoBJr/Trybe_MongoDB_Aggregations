SELECT 
  ...
  CASE
    ...
    WHEN ROUND(SUM(employees.SALARY) / COUNT(*)) > 5800 AND ROUND(SUM(employees.SALARY) / COUNT(*)) <= 7500 THEN 'Pleno'
    WHEN ROUND(SUM(employees.SALARY) / COUNT(*)) > 7500 AND ROUND(SUM(employees.SALARY) / COUNT(*)) <= 10500 THEN 'Sênior'
    ELSE 'CEO'
  END AS Nível
FROM
  hr.employees
