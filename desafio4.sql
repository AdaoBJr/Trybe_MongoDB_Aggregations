...
SELECT j.JOB_TITLE 'Cargo', ROUND(AVG(e.SALARY), 2) 'Média salarial', CASE WHEN AVG(e.SALARY) > 10500 THEN 'CEO' WHEN AVG(e.SALARY) > 7500 THEN 'Sênior' WHEN AVG(e.SALARY) > 5800 THEN 'Pleno' ELSE 'Júnior' END AS 'Senioridade' FROM hr.jobs j INNER JOIN hr.employees e ON j.JOB_ID = e.JOB_ID GROUP BY `Cargo` ORDER BY `Média salarial`, `Cargo`;
